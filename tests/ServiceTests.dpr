...
begin
  if FailClock then
    raise Exception.Create('clock unavailable after service stopped');
  Result := Time;
end;
...
begin
  raise EConvertError.Create('service fault')
end;
...
begin
  ...
  try
    N := Service.StartedCount
  except
    on EFiberUsage do
      Inc(Rejected)
  end;
  try
    N := Service.SkippedCount
  except
    on EFiberUsage do
      Inc(Rejected)
  end;
...
begin
  ...
  Check(OtherService.Stop(10000), 'SERVICE_OTHER_STOP');
  OtherService.Free;
  Teardown;
end;
...
begin
  ...
  if GenerationChange then
    Inc(Driver.Generation)
  else
    Driver.Time := 1000;
  FaultDetected := False;
  try
    Scheduler.RunUntil(1600)
  except
    on EFiberUsage do
      FaultDetected := True
  end;
  ...
  Stopped := False;
  try
    Stopped := Service.Stop(1000)
  except
    on EFiberUsage do
      Stopped := False
  end;
  if GenerationChange then
    Check(Stopped, 'SERVICE_GENERATION_STOP_CLEANUP')
  else
    Check(Stopped, 'SERVICE_BACKWARD_STOP_CLEANUP');
...
begin
  ...
  Probe.Start;
  Probe.WaitFor;
  Check(Probe.Rejected = 9, 'SERVICE_FOREIGN_OWNER');
  Probe.Free;
  Service.Start;
  Rejected := False;
  try
...
begin
  ...
  Rejected := False;
  try
    TFiberService.Create(nil, 1000, Count, nil).Free
  except
    on EFiberUsage do
      Rejected := True
  end;
  ...
  Rejected := False;
  try
    TFiberService.Create(Scheduler, 0, Count, nil).Free
  except
    on ERangeError do
      Rejected := True
  end;
  ...
  Rejected := False;
  try
    TFiberService.Create(Scheduler, 1000, nil, nil).Free
  except
    on EFiberUsage do
      Rejected := True
  end;
...
begin
  TestStopBeforeCallback;
  TestFixedRate;
  TestStopTimeout;
  TestIndependentStop;
  TestGuardsAndFault;
  TestDormantEpochAndArguments;
  TestClockFaultStop(False);
  TestClockFaultStop(True);
