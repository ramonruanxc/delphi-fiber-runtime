...
var S: TPeriodicSchedule; Raised: Boolean; N: Integer; E, P: Int64;
begin
  for N := 1 to 5 do
  begin
    E := 0; P := 1;
    case N of
      ...
      3: P := -1;
      4: E := High(Int64);
      5: begin E := 1; P := High(Int64) end;
    end;
    Raised := False;
    S := nil;
    try
      try S := TPeriodicSchedule.Create(E, P);
      except on ERangeError do Raised := True end;
    finally S.Free end;
...
var S: TPeriodicSchedule; T: TPeriodicTick; Raised: Boolean;
begin
  S := TPeriodicSchedule.Create(100, 100);
  try
    Raised := False;
    try S.TryAcquire(99, T); except on ERangeError do Raised := True end;
    ...
    Raised := False;
    try S.TryAcquire(149, T); except on ERangeError do Raised := True end;
    Check(Raised, 'BACKWARD_IDLE_REJECTED');
    CheckState(S, 200, 0, 0, False, False, 'BACKWARD_IDLE_ATOMIC');
    Raised := False;
    try S.Complete(10000); except on Exception do Raised := True end;
    ...
    Raised := False;
    try S.Complete(249); except on ERangeError do Raised := True end;
    Check(Raised, 'BACKWARD_COMPLETE_REJECTED');
    CheckState(S, 200, 1, 0, True, False, 'BACKWARD_COMPLETE_ATOMIC');
    Raised := False;
    try S.TryAcquire(249, T); except on ERangeError do Raised := True end;
...
begin
  ...
  try
    ...
    Raised := False;
    try S.Complete(High(Int64)); except on ERangeError do Raised := True end;
