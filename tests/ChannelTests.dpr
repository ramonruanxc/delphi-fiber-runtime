...
begin
  ...
  Scheduler.Spawn(Sender, nil);
  Scheduler.Spawn(Drain, nil);
...
begin
  Setup(1);
  Scheduler.Spawn(WaitReceive, nil);
  Scheduler.Spawn(CloseChannel, nil);
...
begin
  ...
  Probe.Start;
  Probe.WaitFor;
  Check(Probe.Rejected = 6, 'CHANNEL_FOREIGN_OWNER');
  Probe.Free;
  Rejected := False;
  try
    TFiberChannel.Create(Scheduler, 0).Free
  except
    on ERangeError do
      Rejected := True
  end;
...
begin
  try
    Check(Channel.Send(AData), 'CHANNEL_MULTIPLE_SENDER');
    Inc(SenderDone);
  finally
    Inc(Cleanups)
  end;
...
var
  Tasks: array[0..2] of TScheduledTask;
  I: Integer;
begin
  Setup(1);
  Channel.TrySend(nil);
  for I := 0 to 2 do
    Tasks[I] := Scheduler.Spawn(IndexedSender, Pointer(I + 1));
  Check(not Scheduler.RunUntil(1), 'CHANNEL_MULTIPLE_PARKED');
  for I := 0 to 2 do
  begin
    Scheduler.WakeTask(Tasks[I]);
    Scheduler.WakeTask(Tasks[I]);
  end;
  ...
  Setup(1);
  for I := 0 to 2 do
    Tasks[I] := Scheduler.Spawn(WaitReceive, nil);
