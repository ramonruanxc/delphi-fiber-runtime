...
begin
  if FailWake then
    raise Exception.Create('injected notify failure')
end;
...
begin
  Check((Sender = Source) and (Value <> nil), 'EVENT_IDENTITY');
  Inc(Handlers);
  try
    if SuspendHandler then
    begin
      Task.Yield;
      Task.Delay(100)
    end
...
begin
  ...
  begin
    Check(Publisher.Stop(1000), 'EVENT_PUBLISHER_STOP');
    Publisher.Free
  end;
  Check(Recipient.Stop(1000), 'EVENT_RECIPIENT_STOP');
  Recipient.Free;
...
begin
  ...
  Check(Source.Publish(Payload), 'EVENT_PUBLISH');
  Payload := nil;
...
var
  Poster: TPoster;
  Refused: Boolean;
  OldToken: TServiceEndpoint;
begin
  Setup;
  Poster := TPoster.Create(True);
  Poster.Token := Source;
  Poster.Value := TPayload.Create;
  Poster.Start;
  Poster.WaitFor;
  Check(Poster.Accepted, 'EVENT_NATIVE_POST_ACCEPTED');
  Poster.Free;
...
const Value: IInterface; Data: Pointer);
begin
  raise EConvertError.Create('event handler fault')
end;
...
begin
  ...
  Probe.Start;
  Probe.WaitFor;
  Check(Probe.Rejected = 4, 'EVENT_FOREIGN_OWNER');
  Probe.Free;
  Refused := False;
  try
...
begin
  ...
  Payload := TPayload.Create;
  Check(Source.Post(Payload), 'EVENT_NOTIFY_POOL_RECOVERED');
  Payload := nil;
...
begin
  ...
  Check(Source.Publish(Payload), 'EVENT_FILL_BEFORE_POST_DELIVERY');
  Payload := nil;
