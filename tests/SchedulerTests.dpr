...
var
  S: TFiberScheduler;
  D: TVirtualDriver;
  Steps, Cleanups: Integer;

procedure Check(V: Boolean; const N: string);
begin
  if not V then
  begin
    WriteLn('ASSERTION FAILED: ', N);
    Halt(1);
  end;
end;
...
begin
  Check(D.Clock = 0, 'SCHEDULER_NONBLOCKING_WAIT');
  Inc(Steps);
end;
...
begin
  ...
  S := TFiberScheduler.Create(2, D);
  A := S.Spawn(Awaiter, nil);
  S.Spawn(Runner, nil);
  S.WakeTask(A);
  S.WakeTask(A);
...
begin
  ...
  TestNativePostRace;
  TestMailboxFairness;
  TestTargetAndInvalid;
  TestClockFault;
  TestDrainParkRace;
  TestDriverFailureRetention;
  TestStopTaskClock;
  TestStopTaskHealthy;
  TestStopTaskNewFault;
  TestPostWakeFailure;
  TestResumeDefault;
  TestTraceDuplicate;
  TestConditions;
  TestResumeBackwardGeneration;
