...
begin
  TFiberService(AData).Execute(ATask)
end;
...
begin
  ...
  if AScheduler = nil then
    raise EFiberUsage.Create('Service requires scheduler');
  AScheduler.CheckOwner;
  if APeriodUs <= 0 then
    raise ERangeError.Create('Service period must be positive');
  if not Assigned(AProc) then
    raise EFiberUsage.Create('Service requires callback');
...
begin
  FScheduler.CheckOwner;
  if FStarted or FCancelled then
    raise EFiberUsage.Create('Service cannot restart');
  ...
  try
    FTask := FScheduler.Spawn(ServiceEntry, Self)
  except
    FSchedule.Free;
    FSchedule := nil;
    raise
...
begin
  ...
  begin
    ...
    if FResumeGeneration <> FScheduler.ResumeGeneration then
    begin
      FSchedule.Rebase(FScheduler.ResumeEpochUs);
      FEpochUs := FSchedule.EpochUs;
      FResumeGeneration := FScheduler.ResumeGeneration;
    end;
    if not ATask.AwaitUntilOrResume(FSchedule.NextDeadlineUs, FResumeGeneration) then
      Continue;
    if FCancelled then
      Exit;
    Now := FScheduler.NowUs;
    if FResumeGeneration <> FScheduler.ResumeGeneration then
      Continue;
    if FSchedule.TryAcquire(Now, Tick) then
    begin
      ...
      begin
        Inc(FCrossings);
        FLastCrossingTick := Tick;
        FLastCrossingFinished := Now;
        FSchedule.CompleteAndRebase(Now);
        FEpochUs := FSchedule.EpochUs;
        FResumeGeneration := FScheduler.ResumeGeneration;
      end
      else
        FSchedule.Complete(Now);
    end;
  end;
end;
...
begin
  ...
  Now := FScheduler.NowUs;
  if Now > High(Int64) - ATimeoutUs then
    Deadline := High(Int64)
  else
    Deadline := Now + ATimeoutUs;
...
begin
  ...
  Result := 0;
  if FSchedule <> nil then
    Result := FSchedule.DiscontinuityCount;
end;
...
begin
  FScheduler.CheckOwner;
  Result := FLastCrossingTick
end;

function TFiberService.GetLastCrossingFinished: Int64;
begin
  FScheduler.CheckOwner;
  Result := FLastCrossingFinished
end;
...
begin
  ...
  begin
    ...
    Result := FSchedule.LastSegment
  else
    Result := FSchedule.CurrentSegment;
end;
