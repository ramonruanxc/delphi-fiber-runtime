...
begin
  inherited Create;
  if AScheduler = nil then raise EFiberUsage.Create('Channel requires scheduler');
  AScheduler.CheckOwner;
  if ACapacity <= 0 then raise ERangeError.Create('Channel capacity must be positive');
...
begin
  ...
  if FWaiters <> 0 then raise EFiberUsage.Create('Channel has active waiters');
  inherited;
end;
...
begin
  ...
  if Result = nil then raise EFiberUsage.Create('Channel wait requires current task');
...
begin
  for I := 0 to High(Waiters) do
    if Waiters[I] = nil then begin
      Waiters[I] := Task; Inc(FWaiters); Result := I; Exit;
    end;
...
procedure TFiberChannel.RemoveWaiter(var Waiters: array of TScheduledTask;
  Slot: Integer);
begin Waiters[Slot] := nil; Dec(FWaiters) end;
procedure TFiberChannel.WakeAll(const Waiters: array of TScheduledTask);
var I: Integer;
begin
  for I := 0 to High(Waiters) do
    if Waiters[I] <> nil then FScheduler.WakeTask(Waiters[I]);
end;
function TFiberChannel.TrySend(AValue: Pointer): Boolean;
begin
  FScheduler.CheckOwner;
  Result := False;
  if FClosed or (FCount = Length(FValues)) then Exit;
  FValues[(FHead + FCount) mod Length(FValues)] := AValue;
  Inc(FCount); WakeAll(FReceivers); Result := True;
end;
function TFiberChannel.Send(AValue: Pointer): Boolean;
var Task: TScheduledTask; Slot: Integer;
begin
  FScheduler.CheckOwner;
  Task := FScheduler.CurrentTask;
  if Task <> nil then Task.CheckCancelled;
  while not FClosed do begin
    if TrySend(AValue) then begin Result := True; Exit end;
    Task := RequireTask;
    Slot := RegisterWaiter(FSenders, Task);
    try Task.Park finally RemoveWaiter(FSenders, Slot) end;
  end;
  Result := False;
end;
...
var Task: TScheduledTask; Slot: Integer;
begin
  FScheduler.CheckOwner; AValue := nil;
  Task := FScheduler.CurrentTask;
  if Task <> nil then Task.CheckCancelled;
  while FCount = 0 do begin
    if FClosed then begin Result := False; Exit end;
    Task := RequireTask;
    Slot := RegisterWaiter(FReceivers, Task);
    try Task.Park finally RemoveWaiter(FReceivers, Slot) end;
  end;
  AValue := FValues[FHead]; FValues[FHead] := nil;
  FHead := (FHead + 1) mod Length(FValues); Dec(FCount);
  WakeAll(FSenders); Result := True;
end;
procedure TFiberChannel.Close;
begin
  FScheduler.CheckOwner;
  FClosed := True; WakeAll(FSenders); WakeAll(FReceivers);
end;
