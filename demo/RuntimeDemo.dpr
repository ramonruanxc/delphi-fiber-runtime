...
uses
  ...
  FiberRuntime.Scheduler, FiberRuntime.Service, FiberRuntime.EventHub;
type
  TSample = record
...
begin
  R := TRun(Data);
  if (Value.Index > Cycles) or (R.Count >= Cycles) then begin Task.Cancel; Task.CheckCancelled; end;
  Slot := R.Count;
  ...
  Inc(R.Count);
  if R.Endpoint.Publish(R.Payload) then Inc(Sent) else Inc(Rejected);
...
begin
  Started := Scheduler.NowUs;
  if (Payload = nil) or ((Payload as IBenchmarkPayload).Size <> PayloadBytes) then begin
    Inc(HandlerFaults); Exit;
  end;
  if CallbackUs > 0 then while Scheduler.NowUs - Started < CallbackUs do;
  Inc(Received);
end;
...
begin if Present then Write(Value) else Write('null'); end;
procedure WriteTrace(const S: TSample);
const Reasons: array[TReadyReason] of string = ('spawn','yield','wake','timer','cancel','resume');
begin
  ...
  Write(',"timer_observed_us":'); Nullable(S.Trace.TimerObservedUs, S.Trace.HasTimerObservation);
  Write(',"ready_enqueued_us":'); Nullable(S.Trace.ReadyEnqueuedUs, S.Trace.HasReadyEnqueue);
  Write(',"resumed_us":'); Nullable(S.Trace.ResumedUs, S.Trace.HasResume);
  Write(',"resume_generation":'); Nullable(S.Trace.ResumeGeneration, S.Trace.HasResume);
  Write(',"ready_reason":"');
  if S.Trace.HasReadyEnqueue then Write(Reasons[S.Trace.ReadyReason]) else Write('inline');
  Write('","ready_generation":');
  if S.Trace.HasReadyEnqueue then Write(S.Trace.ReadyGeneration) else Write(S.Generation);
end;

procedure WriteSegment(const S: TPeriodicSegment);
begin
  if S.Generation < 0 then begin Write('null'); Exit; end;
  Write('{"generation":', S.Generation, ',"epoch_us":', S.EpochUs,
    ',"ended_us":', S.EndedUs, ',"started":', S.StartedCount,
    ',"skipped":', S.SkippedCount, '}');
end;
procedure Run;
var I, J: Integer; EndUs, Candidate: Int64; Receiver: TFiberService; Payload: TPayload;
    Timer: TPlatformTimer; S: TSample;
begin
  ValidateAllocationProbe;
  Services := Option('--services', 8); Cycles := Option('--cycles', 200);
  Capacity := Option('--event-capacity', 256); PayloadBytes := Option('--payload-bytes', 64);
  ...
  if (Services < 1) or (Services > 128) or (Cycles < 1) or (Cycles > 10000) or
     ...
     (CallbackUs < 0) or (CallbackUs > 100000) or (WorkUs < 0) or (WorkUs > 100000) or (AwaitUs < 0) or (AwaitUs > 100000) then
  ...
  if not Receiver.Stop(1000000) then raise Exception.Create('Receiver stop timeout');
  ...
  Write('{"format":"runtime-demo-v2","period_us":1000,"planned_cycles":', Cycles,
    ...
    ',"rejected":', Rejected, ',"disposed":', Disposed, ',"stopped":true,"carrier_threads":1,"warmup_us":10000,',
  ...
  for I := 0 to Services - 1 do begin
    if I > 0 then Write(',');
    if Runs[I].Count > 0 then Candidate := Runs[I].Samples[0].EpochUs
    else Candidate := Runs[I].Service.EpochUs;
    Write('{"epoch_us":', Candidate, ',"segments":{"discontinuity_count":',
      ...
      ',"last":'); WriteSegment(Runs[I].Service.LastSegment);
    Write(',"current":'); WriteSegment(Runs[I].Service.CurrentSegment);
    Write('},"samples":[');
  ...
  Receiver.Free; Hub.Free; Scheduler.Free;
end;
begin
